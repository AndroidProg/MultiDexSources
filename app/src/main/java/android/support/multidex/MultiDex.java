...
public final class MultiDex {
    ...
    public static void install(Context context) {
        ...
        if (IS_VM_MULTIDEX_CAPABLE) {
            //判断VM是否支持Multidex，如果是ART虚拟机的话本身就支持的，MultiDex库则被禁用；
    ...
    private static void expandFieldArray(Object instance, String fieldName, Object[] extraElements) throws NoSuchFieldException, IllegalArgumentException, IllegalAccessException {
        ...
        System.arraycopy(extraElements, 0, combined, original.length, extraElements.length);//多个dex dexN 拷贝新的数组
     ...
     *
     * 清理老的缓存DEX文件
     * context.getFilesDir()获取的路径为      /data/user/0/packageName/files/secondary-dexes
     */
    private static void clearOldDexDir(Context context) throws Exception {
        ...
        if (dexDir.isDirectory()) {
            ...
            File[] files = dexDir.listFiles();  //获取/data/user/0/packageName/files/secondary-dexes下所有文件和目录的绝对路径
            if (files == null) {
            ...
            //循环清空   /data/user/0/packageName/files/secondary-dexes下的所有文件
    ...
    /**
     * 获取DEX路径
     * @return 的文件路径   data/user/0/packageName/code_cache/secondary-dexes
     */
    private static File getDexDir(Context context, File dataDir, String secondaryFolderName) throws IOException {
        ...
        try {
            mkdirChecked(cache); //创建一个   data/user/0/packageName/code_cache
        } catch (IOException var5) {
            cache = new File(context.getFilesDir(), "code_cache");
            mkdirChecked(cache);//出现异常了 创建data/user/0/packageName/files/code_cache
        }

        File dexDir = new File(cache, secondaryFolderName);
        mkdirChecked(dexDir);  //创建一个文件：data/user/0/packageName/code_cache/secondary-dexes  用来存放优化后的dex文件
        return dexDir;
    }
    ...
    private static void mkdirChecked(File dir) throws IOException {
        dir.mkdir();  //建立一级文件夹
        if (!dir.isDirectory()) {  //如果不是一个文件夹，获取父类文件
